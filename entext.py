...
def bold():
    ...
    try:
        ...
        if "bold" in string:
            text.tag_remove("bold", "sel.first", "%s + %sc" %
                            ("sel.first", length))

        else:
            text.tag_add("bold", "sel.first", "%s + %sc" %
                         ("sel.first", length))
...
def italic():
    ...
    try:
        ...
        if "italic" in string:
            text.tag_remove("italic", "sel.first", "%s + %sc" %
                            ("sel.first", length))

        else:
            text.tag_add("italic", "sel.first", "%s + %sc" %
                         ("sel.first", length))
...
def underline():
    ...
    try:
        ...
        if "underline" in string:
            text.tag_remove("underline", "sel.first", "%s + %sc" %
                            ("sel.first", length))

        else:
            text.tag_add("underline", "sel.first", "%s + %sc" %
                         ("sel.first", length))
...
def spell_suggest():
    ...
    webbrowser.open("http://suggest.aspell.net/index.php?word=" +
                    string + "&spelling=american&dict=normal&sugmode=slow")
...
def about():
    ...
    label1.pack()
    label2 = Label(toplevel, text=string2, height=0, width=50,
                   wraplength=500, fg="blue", cursor="hand2")
    label2.pack()
    label2.bind("<Button-1>", git)
    label3 = Label(toplevel, text=string3, height=0, width=50,
                   wraplength=500, fg="blue", cursor="hand2")
    label3.pack()
...
def best_match():
    ...
    res = requests.get('http://suggest.aspell.net/index.php?word=' +
                       word + '&spelling=american&dict=normal&sugmode=slow')
...
text = ScrolledText(master, width=400, height=380, wrap='word', font=(
    "Verdana", 10), highlightthickness=0, bd=2, undo=True, pady=2, padx=3)
...
menu_name = ["File", "Edit", "Clip Board", "Formatting",
             "Help", "Spell suggest", "Morse Translator"]
command_menu_name = ["Synonym", "Meaning", "Spell-Check",
                     "Document Statistics", "Improve Text", "Speech-Aid"]

menu_list = [file_menu, edit_menu, clip_util,
             format_menu, help_menu, spell_menu, morse_translate]
command_menu = [syn, dict_, spell_check_all,
                docu_stats, improve_text, speech_aid]
...
drop_list = [[("New", new), ("Open", open_file), (0,), ("Close", close), ("Save", save), ("Rename", rename)],
             [("Undo", text.edit_undo), ("Redo", text.edit_redo),
              (0,), ("Find", find), ("Find & Replace", find_replace)],
             [("Cut", cut), ("Copy", copy), ("Paste", paste),
              (0,), ("Select All", select_all)],
             [("Bold", bold), ("Italic", italic), ("Underline", underline)],
             [("Help", help_text), ("About", about)],
             [("Get best match", best_match), ("Get all matches", spell_suggest)],
             [("Encrypt to Morse Code", encrypt_morse),
              ("Decrypt from Morse", decrypt_morse)],
             ]
